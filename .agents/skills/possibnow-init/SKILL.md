...
Report the installer's result and any `UNCONFIRMED` commands. If it fails, show the actionable error. After a successful write, review the diff and remind the user to sanitize and commit the shared governance and continuity files with their work. Do not claim that Claude hooks are installed by this project-file installer.
